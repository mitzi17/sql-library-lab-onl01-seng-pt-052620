...
def select_series_title_with_most_human_characters
  "SELECT series.title, COUNT(characters(species)) FROM series JOIN authors ON series.author_id = authors.id JOIN characters ON author.id = characters.author_id WHERE characters.species = 'human' ORDER BY COUNT(characters(species)) DESC LIMIT 1;"
end

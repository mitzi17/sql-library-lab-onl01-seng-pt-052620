...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series JOIN books ON series.book_id = books.id JOIN characters ON authors.id = characters.author_id WHERE characters.species = 'human' ORDER BY COUNT(characters.species) DESC LIMIT 1;"
end
